...
def main(config_file):
    ga_object = run_GA(config_file)
    values = [[1000, 0.10], [1100.0, 0.1], [1200.0, 0.1], [1300.0, 0.1]]
    ga_object.cases[0].plot_result([1.15080817e+03, 3.09598847e-02])
  #  ga_object.cases[0].give_values(values[0])
    # ga_object.result = [1100, 0.11]
    # ga_object.post_processing()
   # ga_object.cases[0].calc_fitness()
   # res = ga_object.calc_fitness(values)
   # print(res)
    # ga_object.cases[0].plot_result(values[0])
    # ga_object.cases[0].give_values(values[0])
    # res = ga_object.cases[0].calc_fitness()
    # ga_object.cases[1].give_values(values[1])
    # res2 = ga_object.cases[1].calc_fitness()
    # ga_object.cases[1].give_values(values[0])
    # res3 = ga_object.cases[1].calc_fitness()
    # print(res)
    result = ga_object.run_GA(500.0)
    # ga_object.result = values[0]
    ga_object.post_processing()


if __name__ == '__main__':
    config_files = [
        r'd:\projecten\calibration\config_taw.yml',
